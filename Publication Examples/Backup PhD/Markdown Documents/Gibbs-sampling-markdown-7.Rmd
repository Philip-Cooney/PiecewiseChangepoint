...
knitr::include_graphics("C:/Users/phili/OneDrive/PhD/Plots/DAG.jpg")
```
...
output <- gibbs.changepoint_chains_optim(df, num.breaks = 2, n.iter = 100, burn_in = 0, num.chains = 4)
...
output_final <- gibbs.changepoint_chains_optim(df, num.breaks = 2, n.iter = 10002, burn_in = 100, num.chains = 1)
...
```{r post-change,  fig.cap = 'Posterior distributions for changepoints', echo = FALSE, fig.align='center',echo=FALSE,warning=FALSE, fig.asp=.6, fig.width = 15}
...
The posterior distributions for the hazards are presented in Figure \@ref(fig:post-haz-BUGS) with the posterior distribution for the changepoints in Figure \@ref(fig:post-change-BUGS). The results are generally similar to the results obtained using the optimized Gibbs Sampler, however, distribution at changepoint is skewed based the true value of 5 years. This is possibly related to the fact that the model has not been run to convergence. 


```{r post-haz-BUGS, fig.cap = 'Posterior hazards', echo = FALSE, fig.asp=.75, fig.align='center',echo=FALSE,warning=FALSE}
...
```{r post-change-BUGS,  fig.cap = 'Posterior distributions for changepoints', echo = FALSE, fig.asp=.75, fig.align='center',echo=FALSE,warning=FALSE, fig.width= 9}
...
Figure \@ref(fig:post-diff-surv) below compares the results for the different survival models against the true data. Although the graph lacks clarity, the piecewise exponential (purple) appears to quite closely follow the true distribution. 


```{r post-diff-surv, fig.cap = 'Plot of the different survival model versus the true data', echo = FALSE, out.width='80%', fig.asp=.75, fig.align='center',echo=FALSE,warning=FALSE, cache=TRUE}



output_3 <- gibbs.changepoint_chains_optim(df[,c("time", "status")], num.breaks = 2, n.iter = 5001, burn_in = 100, num.chains = 1, max_predict = max(df$time_event),
...
Figure \@ref(fig:true-extrap) shows the true survival without censoring (blue) with the purple line representing the posterior mean survival. 

```{r true-extrap, echo = FALSE, cache=TRUE}

output_3[[3]]

```

If we grossly overspecify the number of changepoints in the model (i.e. by running a 10 changepoint model) the extrapolation is much further away from the true survival. Figure \@ref(fig:mispecified) shows that there is another changepoint after the true changepoint at 3 years, meaning that the extrapolation is based on fewer observations.
...
```{r mispecified, echo = TRUE, cache=TRUE}
...
# Scenarios in which piecewise outperform other survival models

As discussed in the previous sections, the algorithm has reasonable sensitivity for identifying the changepoints and consequently the hazards. Following this it is useful to identify situations in which the piecewise model can produce the optimal fit to the observed data along with plausible extrapolations. 

Unsurprising this occurs when the data-generating process is piecewise exponential and we have information on the number of changepoints. If a large change in hazard is observed and there is a reasonable about of data observed after the hazard, then the piecewise exponential model will fit both the observed data and the unobserved data after the "end of trial". We can statistically quantify how well the model fitted to the observed data explains the uncensored (true) data by evaluating the log-likelihood of the true data using the model parameters generated from the censorsed observations. 

Rather than estimating all of the standard survival models in a Bayesian framework it was considered apporiate to fit the data using standard Frequentist methods. In other to make an appropriate comparison between the changepoint model (estimated under Bayesian methods) and the frequentist survival models, the _mode_ of the changepoint likelihood was selected. Because uniform priors were chosen for the changepoints (and vague prior for the other parameters), the mode of the likelihood function (the mode and median are very similar) should be an appropriate comparison with the likelihoods evaulated from the Frequentist methods.    

## Models with one changepoint

Below we have considered a 1 changepoint model in which the hazard is high (0.75) in the early period but falls dramatically after half a year. There are 500 observations and the study window is up until 3.5 years. We have approximately 150 censored during the study with another 100 censored observations at the end of the study.  
...
#survfit(formula = Surv(df$time_event, rep(1, nrow(df))) ~ 1)
...
#Just done for the cumulative hazard plots
surv_true_5 <- survfit(Surv(time,status)~obs, data = df4[which(df4$time <4),])
...
output_5 <- gibbs.changepoint_chains_optim(df[,c("time", "status")], num.breaks = 1, n.iter = 10001, burn_in = 100,
                                           num.chains = 1, max_predict = max(df$time_event),
                                           obs_true = df$time_event )


```

```{r cum-haz-2, fig.cap = 'Plot of the cumulative hazards', echo = FALSE}

mean_vec <- unlist(data.frame(output_5[[4]][,1]) %>%
                     dplyr::summarise_all(list(mean = mean)))

cum_haz_plot <- ggsurvplot(surv_true_5, conf.int = TRUE, 
                           palette = c("#FF9E29", "#86AA00"),
                           risk.table = TRUE, risk.table.col = "strata",
                           fun = "cumhaz")

cum_haz_plot$plot + geom_vline(xintercept = mean_vec, 
                               linetype = "dotted", 
                               color = "blue", size=1.5 )
```

Table \@ref(tab:summary-tab-logLik) presents the log-likelihood for each statistical model, in descending order (larger values indicate better fit).

```{r summary-tab-logLik, echo= FALSE, warning = FALSE}
knitr::kable(output_5[[6]], digits = 2, caption = 'Ranking of the log-likelihood for each survival model') %>%
kable_styling(latex_options = c("striped", "hold_position"))
```

We can also consider how the model performs if we include more than the actual number of changepoints. Even if the number of changepoints is overspecified, if there is sufficient data before the end of study to elucidate the correct hazard, the extrapolation should remain unbiased.

```{r, echo = FALSE, warning = FALSE, cache= TRUE }
output_6 <- gibbs.changepoint_chains_optim(df[,c("time", "status")], num.breaks = 2, n.iter = 10001, burn_in = 100,
                                           num.chains = 1, max_predict = max(df$time_event),
                                           obs_true = df$time_event )
```

```{r summary-tab-logLik2, echo= FALSE, warning = FALSE}
knitr::kable(output_6[[6]], digits = 2, caption = 'Ranking of the log-likelihood for each survival model') %>%
kable_styling(latex_options = c("striped", "hold_position"))
```

Another scenario is where the the hazard is initially high then falls and then rises rapidly again. In this situation the hazard is 0.75 for half a year, falls to 0.2 until year 2.5 and then rises to 0.5.
...
surv_true_5 <- survfit(Surv(time,status)~obs, data = df4)

```

```{r, echo = FALSE, warning = FALSE, cache = TRUE}

output_7 <- gibbs.changepoint_chains_optim(df[,c("time", "status")], num.breaks = 2, n.iter = 10001, burn_in = 100,
                                           num.chains = 1, max_predict = max(df$time_event),
                                           obs_true = df$time_event )
```


```{r summary-tab-logLik3, echo= FALSE, warning = FALSE}
knitr::kable(output_7[[6]], digits = 2, caption = 'Ranking of the log-likelihood for each survival model') %>%
kable_styling(latex_options = c("striped", "hold_position"))
```

## Changepoint analysis in multiple populations

Although we see the model performs well in the situation that the data are distributed as piecewise exponential observations, a related situation (which is aruguably more plausible in real life), is the presence of multiple populations each with a different underlying constant hazard.

In the situation below we consider a population (n = 500) with 2 subgroups; one with a hazard of 0.75 (n1 = 150) and another population with a hazard of 0.2 (n2 = 350). We wish to consider censoring within the observable period (approximately 150 observations), along with all observations still in the risk set being censored at 3.5 years. 
...
time_1 <- rexp(n1, rate = rate1)
time_2 <- rexp(n2, rate = rate2)
samp <- c(time_1, time_2)

max_time <- 3.5
n_cens_req <- 150
samp_cens <- rexp(n_cens_req*2, rate = rate2)
samp_cens <- sapply(samp_cens, FUN = min, max_time)
samp_cens <- sample(c(samp_cens,rep(max_time,n- n_cens_req*2))) # Randomized vector 
#http://www.cookbook-r.com/Manipulating_data/Randomizing_order/
...
output_8 <- gibbs.changepoint_chains_optim(df[,c("time", "status")], num.breaks = 1, n.iter = 10000, burn_in = 100,
                                           num.chains = 1, max_predict = max(df$time_event),
                                           obs_true = df$time_event )
...
From Table \@ref(tab:summary-tab-logLik4) we can see that the Weibull and piecewise models provide very similar fit to the data. 

```{r summary-tab-logLik4, echo= FALSE, warning = FALSE}
knitr::kable(output_8[[6]], digits = 2, caption = 'Ranking of the log-likelihood for each survival model') %>%
kable_styling(latex_options = c("striped", "hold_position"))
```

Finally we consider a 3 subgroups with hazards 0.9, 0.2 and 0.5 respectively, with sample sizes 150,350,150. We censored approximately 200 observations within the study period and all observations still within the Risk Set. 
...
time_1 <- rexp(n1, rate = rate1)
time_2 <- rexp(n2, rate = rate2)
time_3 <- rexp(n3, rate = rate2)
samp <- c(time_1, time_2, time_3)
...
output_9 <- gibbs.changepoint_chains_optim(df[,c("time", "status")], num.breaks = 2, n.iter = 10000, burn_in = 100,
                                           num.chains = 1, max_predict = max(df$time_event),
                                           obs_true = df$time_event )

```

We observe a similar trend with the log-likelihoods for the 3 population model with the Weibull and Piecewise Exponential model providing very similar fit (both visually and by log-likelihood). However, it does not appear that a piecewise exponential model is offers any particular improvement for the survival extrapolation of this population.

```{r summary-tab-logLik5, echo= FALSE, warning = FALSE}
knitr::kable(output_9[[6]], digits = 2, caption = 'Ranking of the log-likelihood for each survival model') %>%
kable_styling(latex_options = c("striped", "hold_position"))
```
...
## Non-constant hazards plus constant hazard after a changepoint

Probably the most realistic scenario is a situation where a hazard is observed to change overtime and then become constant at a chanagepoint. If we consider a situation when the hazard increases up until 2 years and then becomes a constant value. In this example we truncate the time at 3.5 years.

```{r, echo = FALSE, warning= FALSE, results= 'hide', cache = TRUE}

scale = 4 #Inverse Rate
shape = 1.5 #(inceasing hazard) >1 constant = 1

#Changepoint at time 2
changetime <- 2
cdf_change <- pweibull(changetime, shape = shape, scale = scale, lower.tail = TRUE, log.p = FALSE)
haz_change <- dweibull(changetime, shape = shape, scale = scale)/(1-cdf_change)


expo_haz <- haz_change #1/scale
#cumulative distribution function which is just ln(St)

(1-cdf_change)*(1-pexp(13, rate = expo_haz))


St_expo_change <- 1-pexp(changetime, rate = expo_haz)
haz_change <- dexp(changetime, rate = expo_haz)/St_expo_change

increments <- 250
p.vals <- seq(from= 0, to =1, length.out= increments)
haz_time <- time_sim <- rep(NA,  increments)


#
for(i in 1:increments){

if(p.vals[i] < cdf_change){
  #St <- 1- pweibull(p, shape, scale = scale, lower.tail = TRUE, log.p = FALSE)
  time_sim[i] <- qweibull(p.vals[i], shape = shape, scale = scale, lower.tail = TRUE, log.p = FALSE)
  haz_time[i] <- dweibull(time_sim[i], shape = shape, scale = scale)/(1-pweibull(time_sim[i], shape = shape, scale = scale))
}else{

 p.new <-  (p.vals[i]-cdf_change)/(1.0000000-cdf_change)
 time.expo.new <- qexp(p.new, rate = expo_haz)
 time_sim[i] <- changetime + time.expo.new
 haz_time[i] <- dexp(time_sim[i], rate = haz_change)/(1-pexp(time_sim[i], rate = haz_change))
 }
}

```

Knowing the parameteric form of the model, the hazards are presented below.

```{r haz-1, fig.cap = 'Plot of the  hazards', echo = FALSE}

plot(x= time_sim, y = haz_time, 
     type = "l", xlim = c(0,5), ylab = "Hazard", xlab = "Time")

```

```{r, echo = FALSE, warning= FALSE, results= 'hide', cache = TRUE}
df <-data.frame(time_event = time_sim) %>% mutate(status = ifelse(time_event <=time_trunc,1,0)) %>%
    mutate(time = ifelse(time_event <=time_trunc,time_event, time_trunc))
    
df <- df[which(!is.nan(df$time)),]
df <- df[-1,] 
```


```{r echo = FALSE, warning= FALSE, results= 'hide', cache = TRUE}
output_8 <- gibbs.changepoint_chains_optim(df[,c("time", "status")], num.breaks = 4, n.iter = 10000, burn_in = 100,
                                           num.chains = 1, max_predict = 40, obs_true = df$time_event[which(df$time_event<40)])

```

Although the model fit the data well choice of changepoints is arbitary. As clearly shown in \@ref(fig:haz-1), it is not possible to approximate the shape of the hazard with 1 or 2 constant hazards. Making an educated guess, it is plausible that 4 seperate hazards could adequately model hazard, therefore a 3 changepoint model was chosen.
...
Although the 3 changepoint piecewise model performs well, it would be better to be able to automate the number of changepoints as this choice could be considered quite subjective. This will be the focus of the next chapter.
