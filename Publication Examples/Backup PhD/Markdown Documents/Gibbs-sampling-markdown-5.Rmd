...
output_5 <- gibbs.changepoint_chains_optim(df[,c("time", "status")], num.breaks = 1, n.iter = 10000, burn_in = 100,
...
knitr::kable(summary(output_5[[6]]), digits = 2, caption = 'Ranking of the log-likelihood for each survival model') %>%
kable_styling(latex_options = c("striped", "hold_position"))
```
...
```{r, echo = FALSE, warning = FALSE, cache= TRUE }
output_6 <- gibbs.changepoint_chains_optim(df[,c("time", "status")], num.breaks = 2, n.iter = 10000, burn_in = 100,
                                           num.chains = 1, max_predict = max(df$time_event),
                                           obs_true = df$time_event )
```

```{r summary-tab-logLik2, echo= FALSE, warning = FALSE}
knitr::kable(summary(output_6[[6]]), digits = 2, caption = 'Ranking of the log-likelihood for each survival model') %>%
kable_styling(latex_options = c("striped", "hold_position"))
```
...
output_7 <- gibbs.changepoint_chains_optim(df[,c("time", "status")], num.breaks = 2, n.iter = 10000, burn_in = 100,
                                           num.chains = 1, max_predict = max(df$time_event),
                                           obs_true = df$time_event )
```
...
knitr::kable(summary(output_7[[6]]), digits = 2, caption = 'Ranking of the log-likelihood for each survival model') %>%
kable_styling(latex_options = c("striped", "hold_position"))
```
...
In the situation below we consider a population with 2 subgroups; one with a hazard of 0.75 and another population with a hazard of 0.2.


Next steps
*Evaluate the log-likelihood of these to show that the piecewise exponential is the best.

*Simulate exponential times from two seperate populations and see if how the models performs there.
